...
Floatbox.prototype.getAllOptions = function() {
	...
	if ((t.lclTheme = t.theme) === 'auto') {
		t.lclTheme = t.itemToShow.type === 'img' ? 'black' : (t.itemToShow.type.indexOf('media') === 0 ? 'black' : 'black');
	}
	if (!t.doAnimations) {
		...
		if (item.type === 'inline') {
			...
			fb.xhr.getResponse(item.href, function(xhr) {
				if ((xhr.status === 200 || xhr.status === 203 || xhr.status === 304) && xhr.responseText) {
					...
					t.setInnerHTML(t.fbContent, '<p style="color:#fff; background:#000; margin:1em; padding:1em;">' +
